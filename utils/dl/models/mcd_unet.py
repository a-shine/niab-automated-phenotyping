"""
U-Net model with Monte Carlo (MC) dropout for uncertainty quantification (UQ).

The model is based on the U-Net architecture with MC dropout layers. The model
is implemented using PyTorch.

The U-Net architecture was first introduced in the paper "U-Net: Convolutional
Networks for Biomedical Image Segmentation" by Ronneberger et al. (2015).

The MC dropout layers are used to estimate the uncertainty in the model
predictions. The MC dropout layers are applied to the input and output of each
convolutional layer in the network.

The implementation has been modified from https://github.com/samirapakravan/segmentation_uq/tree/main
"""
