...
training_dataset, validation_dataset = random_split(
    data_processed, [train_size, val_size]
)
...
if MODEL_NAME == "unet":
    ...
    raise ValueError(
        "Model name not recognised. Please choose from 'unet', 'deeplab' or 'mcdunet'."
    )
...
os.makedirs(
    f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}", exist_ok=True
)

# write model arch and dataset used to a file
with open(
    f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}/desc.txt", "w"
) as f:
...
val_metrics_df = pd.DataFrame(
    [], columns=["epoch", "train_loss", "val_loss", "val_iou_score", "val_f1"]
)
...
for t in range(EPOCHS):
    print(f"Epoch {t + 1}\n-------------------------------")
    train_metrics = fit(
        train_dataloader, model, loss_fn, optimizer, None, device, log_freq=2
    )
        ...
        [
            {
                ...
                "val_iou_score": val_metrics["iou_score"],
    ...
    val_metrics_df = pd.concat(
        [val_metrics_df if not val_metrics_df.empty else None, metrics_df],
        ignore_index=True,
    )

    val_metrics_df.to_csv(
        f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}/val_metrics.csv"
    )
    ...
    if val_metrics["loss"] < best_loss:
        ...
        torch.save(
            model.state_dict(),
            f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}/best_model.pth",
        )
...
plt.legend()
plt.savefig(
    f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}/training_loss.png"
)
...
plt.legend()
plt.savefig(
    f"/home/users/ashine/gws/niab-automated-phenotyping/models/{job_id}/val_metrics.png"
)
